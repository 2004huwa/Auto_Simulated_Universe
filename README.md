...
info.txt中第一行保存了模拟宇宙开局选的角色，建议改成自己的配队（最新版本成为废案，无需修改第一行，只需自行在游戏内选择好要打的世界和配队即可），1表示第一个角色。最好在一号位选远程角色（艾丝妲、三月七）方便开怪。第二行是校准数据，不要改第二行！

第三行是宇宙的难度，如果你要打难度1就改成1保存。第四行是命途选择，默认巡猎，可以直接修改为其它命途。
...
在设置中选择自己想要的难度和命途，配队请用单空格隔开四个数字（已是废案，同上），1表示第一个角色（编号规则示意见下图）。最好在一号位选远程角色（艾丝妲、三月七）方便开怪。
...
选祝福的逻辑是ocr识别对应的命途祝福，事件基本都会跳过，最后一层不会强化祝福，奇物随机选。
